...
void VirtualApplication::initialize()
{
	...
	resMgrModule = check_and_cast <ResourceManager*>(parent->getSubmodule("ResourceManager")->getSubmodule("ResourceManager"));
	enMgrModule = check_and_cast <VirtualEnergyManager*>(parent->getSubmodule("ResourceManager")->
	...
	if (!resMgrModule || !mobilityModule || !radioModule || !enMgrModule)
	...
	/* Modified for GreenCastalia integration
	 * ------------------------------------------------------------------------
	 * Send the STARTUP message to the Resource Manager module so that the node 
	 * starts operation if there is enough energy available. 
	 * ------------------------------------------------------------------------
	 * Note that we send the message to the Resource Mgr through
	 * the unconnected gate "powerConsumption" using sendDirect()
	 */
	sendDirect(new cMessage("Resource Mgr [STARTUP]", NODE_STARTUP),
		    startup_delay, 0, resMgrModule->getParentModule(), "powerConsumption");
